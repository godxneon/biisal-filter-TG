...
import pyrogram
from info import SETTINGS, MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, PICS, NOR_IMG, LANGUAGES, QUALITIES, YEARS, SEASONS, AUTH_CHANNEL, IMDB, IMDB_TEMPLATE, FILE_CAPTION, DELETE_TIME, SUPPORT_GROUP
...
async def group_search(client, message):
    ...
    if message.chat.id == SUPPORT_GROUP :
                ...
                if total != 0:
                    ...
                    msg = await message.reply_text(script.SUPPORT_GRP_MOVIE_TEXT.format(message.from_user.mention(), total), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ 😉' , url='https://t.me/KLMovieGroup')]]))
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            if settings["auto_delete"]:
                ...
                await message.reply_text(cap + links, protect_content=True, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)
    else:
        k = await message.reply_text(cap + links, protect_content=True, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML, reply_to_message_id=message.id)
        if settings['auto_delete']:
            await asyncio.sleep(DELETE_TIME)
            await k.delete()
            try:
